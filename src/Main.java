...
public class Main extends PApplet {
    ...
    public void mousePressed() {
        if (mouseButton == LEFT) {
            if (gui.currentScreen == GUI.SCREEN.PRELOGIN) {
                ...
                if (!gui.tflogin1.mouseOverTextField(this) && gui.tflogin1.selected)
                    gui.tflogin1.keyPressed('0', ENTER);
                if (!gui.tflogin2.mouseOverTextField(this) && gui.tflogin2.selected)
                    gui.tflogin2.keyPressed('0', ENTER);
                ...
                if (!gui.tfsignup1.mouseOverTextField(this) && gui.tfsignup1.selected)
                    gui.tfsignup1.keyPressed('0', ENTER);
                if (!gui.tfsignup2.mouseOverTextField(this) && gui.tfsignup2.selected)
                    gui.tfsignup2.keyPressed('0', ENTER);
                if (!gui.tfsignup3.mouseOverTextField(this) && gui.tfsignup3.selected)
                    gui.tfsignup3.keyPressed('0', ENTER);
                if (!gui.tfsignup4.mouseOverTextField(this) && gui.tfsignup4.selected)
                    ...
                    for (int i = 0; i < gui.tfsced.length; i++) {
                        if (i != 2 && i != 4) {
                            ...
                            if (!gui.tfsced[i].mouseOverTextField(this) && gui.tfsced[i].selected)
                                gui.tfsced[i].keyPressed('0', ENTER);
                        }
                    }
            ...
            if (gui.currentScreen == GUI.SCREEN.SCENESELECTOR){
                for (int i = 15 * gui.page; i < Math.min(15 * (gui.page + 1), gui.scenes.size()); i++) {
                    if (gui.scenes.get(i).mouseOverButton(this)) {
                        print ("Right Button in button "+gui.scenes.get(i).bText);
                        if (gui.scenes.get(i).mouseOverButton(this) && gui.scenes.get(i).state == STATE.NORM){
    ...
    public void deleteScene(Scene sc) {
        ...
        for (int i = 0; i < scenes.size(); i++) {
            int k = scenes.get(i).ID % 5;
            int j = (scenes.get(i).ID / 5) % 3;
